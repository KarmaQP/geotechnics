...
class PhasePlastic:
  ...
  def zero_rows(M, rows):
    diag = sp.sparse.eye(M.shape[0]).tolil()
    diag_2 = sp.sparse.eye(M.shape[0]).tolil()
    ...
    for r in rows:
      diag_2[r, r] = 2
    A += diag_2 - sp.sparse.eye(M.shape[0])
    return A
    ...
    for i in self.phase['lines']:
      ...
      for j, row in enumerate(bound):
        ...
        if row[1] == 1:  # граничное условие по Y
          boun_row.append(2 * j + 1)

      k_glob = self.zero_rows(k_glob, boun_row)
  ...
  @staticmethod
  def solve(k_glob, r_glob, u_0):
    d_x_vector, exit_code = sp.sparse.linalg.bicg(
      k_glob, r_glob, atol=1e-4, x0=u_0)
    print()
    print('Ошибка решения СЛАУ:', exit_code)
    if exit_code != 0:
      d_x_vector = np.linalg.solve(k_glob.toarray(), r_glob)
    return d_x_vector

  def sigma_calc(self, sigma_int, index_last, d_x):
    delta = {}
    for i in self.phase['soils']:
      if i['phaseActivity']:
  ...
  def plastic_solve(self):
    ...
    # Код ошибки
    collapse = 0
    # Вектор внутренних сил на шаге расчета
    self.d_f_int = np.zeros_like(f_int)
    ...
    while m_stage < 1:
      ...
      d_f = alfa * f
      # Определение вектора внешних сил на шаге
      d_f_ext = d_f + 0
      ...
      f_int_i = np.zeros_like(d_f_ext)
      u_i = np.zeros(2 * len(self.list_coor[:, 0]), dtype=np.float64)
      u_i += u_s
      u_s = np.zeros(2 * len(self.list_coor[:, 0]), dtype=np.float64)
      ...
      index_0_last = {}
      index_0_last.update(self.index)
      GE_counter = 0
      GE_counter_2 = 0

      mult = 1
      sigma_int = {}
      sigma_int.update(self.sigma)

      # Начинается цикл по итерациям
      while (global_error >= self.tolerated_error or local_error >= max_inaccurate_point) and (iter < 26):

        # Переопределяем матрицу жесткости
        k_s = self.k_glob_sum(sigma_int, index_last)

        # Определяем перемещение на итерации
        u_i = self.solve(k_s, r, u_i)
        # Суммируем перемещения по итерациям
        u_s += u_i
        # Вычисляем напряжения и пластические точки из модели грунта
        sigma_int, index_last, tau, sigma_elast = self.sigma_calc(
          sigma_int, index_last, u_i)
        # Вычисляем вектор внутренних сил из напряжений
        f_int_i = self.r_internal(sigma_int) - f * (m_stage - alfa) - f_int
        # Определяем вектор сил на итерации (вычитаем из вектора внешних сил вектор внутренних)
        r = d_f - f_int_i
        # Calculation local error
        local_error = self.local_error_check(sigma_int, sigma_elast, tau)
        # Алгоритм сброса расчета шага при низкой сходимости
        new_GE = self.global_error_check(r, f * m_stage)
        if (global_error - new_GE) / (global_error - 0.04) < 0.5 and global_error - new_GE > 0:
          GE_counter_2 += 1
          if GE_counter_2 > 1:
            if mult * 1.5 < 2:
              mult = 1.5 * mult
        ...
        global_error = self.global_error_check(r, f * m_stage)
        if global_error > 0.5:
          iter = 30
        ...
        print('\r', '| Iteration:', iter, '| Global error:', round(global_error, 4),
              '| loc_error_points:', local_error, '| MaxInaccuratePoint', max_inaccurate_point, '| U_max', max(u_s), '| U_min', min(u_s), end='')
        # Ускорение схождения
        if iter > 5:
          r = 1.2 * r
        elif iter > 9:
