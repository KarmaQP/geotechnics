...
class ModelLE:
    ...
    def sigma_check(self, sig, eps, index):
        ...
        sig1 = - 1 / 2 * ((sig_new[0] + sig_new[1]) - ((sig_new[0] - sig_new[1]) ** 2 + 4 * sig_new[2] ** 2) ** 0.5)
        sig3 = - 1 / 2 * ((sig_new[0] + sig_new[1]) + ((sig_new[0] - sig_new[1]) ** 2 + 4 * sig_new[2] ** 2) ** 0.5)
        tau = (sig1 - sig3) / 2
...
class ModelMC:
    ...
    def sigma_check(self, sig, eps, index):
        ...
        sig_1 = - 1 / 2 * ((sig_new[0] + sig_new[1]) - ((sig_new[0] - sig_new[1]) ** 2 + 4 * sig_new[2] ** 2) ** 0.5)
        sig_3 = - 1 / 2 * ((sig_new[0] + sig_new[1]) + ((sig_new[0] - sig_new[1]) ** 2 + 4 * sig_new[2] ** 2) ** 0.5)
        ...
        sigma_internal[0] = - sigma1 * (np.cos(alpha) ** 2) - sigma3 * (np.sin(alpha) ** 2)
        sigma_internal[1] = - sigma1 * (np.sin(alpha) ** 2) - sigma3 * (np.cos(alpha) ** 2)
