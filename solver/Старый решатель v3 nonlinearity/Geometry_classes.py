...
class FE6:
    ...
    def square_calc(self):
        ...
        s = abs((x_loc[1] - x_loc[0]) * (y_loc[2] - y_loc[0]) - (x_loc[2] - x_loc[0]) * (y_loc[1] - y_loc[0])) / 2
        return s
    ...
    def matrix_csc_transform(self, k_loc):
        ...
        transf_matrix = sp.sparse.csc_matrix((data, (cols, rows)), shape=(num_nodes * 2, num_nodes * 2))
    ...
    def k_matrix_calc(self):
        ...
        k_loc = 1 / 3 * s * (b_1.T @ d1 @ b_1 + b_2.T @ d2 @ b_2 + b_3.T @ d3 @ b_3)
    ...
    def r_vector_calc(self):
        ...
        for i in range(3):
            r_vector[2 * self.list_nodes[i + 3] + 1] = - self.material.weight * s * 1 / 3
        return r_vector
    ...
    def sigma_calc(self, delta):
        ...
        sigma_int[0], index[0], tau[0], sigma_elast[0] = self.material.sigma_check(self.sigma[0], eps1, self.index[0])
        sigma_int[1], index[1], tau[1], sigma_elast[1] = self.material.sigma_check(self.sigma[1], eps2, self.index[1])
        sigma_int[2], index[2], tau[2], sigma_elast[2] = self.material.sigma_check(self.sigma[2], eps3, self.index[2])
...
class Polygon:
    ...
    def stiff_matrix(self):
        ...
        stiff_matrix = sp.sparse.csc_matrix(([0], ([0], [0])) ,shape=(num_nodes * 2, num_nodes * 2))
        # Суммирование глобальной матрицы полигона
        for i in range(len(self.list_nodes[:, 0])):
            stiff_matrix += FE6(self.list_coor, self.list_nodes[i], self.material,
    ...
    def r_glob_vector(self):
        ...
        for i in range(num_el):
            r_glob += FE6(self.list_coor, self.list_nodes[i], self.material).r_vector_calc()
        return r_glob
    ...
    def epsilon_calc(self, delta):
        ...
        for i in range(num):
            epsilon[3 * i: 3 * i + 3] = FE6(self.list_coor, self.list_nodes[i]).epsilon_calc(delta[i])
        return epsilon
    ...
    def r_internal(self, sigma):
        ...
        num_el = len(self.list_nodes[:, 0])
        for i in range(num_el):
            r += FE6(self.list_coor, self.list_nodes[i]).r_internal(sigma[3 * i: 3 * i + 3])
        return r
