...
class StageConstruction:
  ...
  def deactive_nodes(self, phase_num):
    ...
    list_deactive_nodes = []
    # Находим узлы выключенных полигонов
    for i in self.phases[phase_num]['data']['soils']:
      if not i["phaseActivity"]:
        list_deactive_nodes.append(self.list_polygon[i['name']])
    ...
    list_active_nodes = self.list_polygon
    # Удаляем выключенные полигоны
    for i in self.phases[phase_num]['data']['soils']:
      if not i["phaseActivity"]:
        list_active_nodes = self.key_remove(self.list_polygon, i['name'])
    # Объединяем значения словаря в массив
    list_active_nodes = list(list_active_nodes.values())
    v = np.zeros((1, len(list_active_nodes[0][0])), dtype=np.int_)
    for i in range(len(list_active_nodes)):
      v = np.vstack([v, list_active_nodes[i]])
    list_active_nodes = np.delete(v, 0, axis=0)
  ...
  def list_active_nodes(self, phase_num):
    ...
    for i in self.phases[phase_num]['data']['soils']:
      if not i["phaseActivity"]:
        list_active_nodes = self.key_remove(self.list_polygon, i['name'])
    # Объединяем значения словаря в массив
    list_nodes = list(list_active_nodes.values())
    ...
    for i in list_deactive_nodes:
      if (list_nodes > i).any():
        list_nodes = list_nodes - np.array((list_nodes > i), dtype=np.int_)
    return list_nodes
