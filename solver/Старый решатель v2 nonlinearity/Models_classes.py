...
class ModelMC:
  ...
  def d_matrix(self, sigma, index):
    ...
    # Разрыв в 2-х направлениях или разрыв + Кулон-Мор, в данном случае напряжения предопределены и ставим
    # хоть какую-то жесткость для того, чтоб не схлопнулись алгоритмы
    if index == 3 or index == 4:
      D = (self.d_e_matrix() - self.f2_ep(sigma[0], sigma[1], sigma[2])) * 0.01
